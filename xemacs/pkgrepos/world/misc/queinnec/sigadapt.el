;;; $Id: sigadapt.el,v 1.25 2005/11/01 15:23:05 queinnec Exp $
;;; Copyright (C) 1997-2002 by C.Queinnec (University of Paris 6)

;;; LCD Archive Entry:
;;; sigadapt|Christian Queinnec|Christian.Queinnec@lip6.fr|
;;; Select signature depending on mailee.|
;;; $Date: 2005/11/01 15:23:05 $|$Revision: 1.25 $|
...
;;; BBDB 1.50. As of June 25, 2002, the sigadapt package is known to
;;; work with Emacs 21.1.1, vm and BBDB 2.35.

;;; {{{ Repository
;;; Bugs, remarks etc should be sent to
;;;     Christian.Queinnec@lip6.fr
;;; Newer versions will be sent to the LCD Archive but may appear earlier on:
;;;     http://www-spi.lip6.fr/~queinnec/Miscellaneous/sigadapt.el
...
(defsignature paracamplus
  "\n
\tC. Queinnec
\tGerant de la societe Paracamplus
\n" )

;;; Should now be encoded as a mime part:
(defsignature vcard
  "\n
BEGIN:vcard
VERSION:2.1
FN:Christian Queinnec
N:Queinnec;Christian
ORG;quoted-printable:Universit=C3=A9 Pierre et Marie Curie;LIP6
ADR:;;4, place Jussieu;Paris;;75252;France
EMAIL;internet:Christian.Queinnec@lip6.fr
ROLE:Directeur de la licence d'informatique de l'UPMC
UID:9152
TEL;work:+33 1 44 27 40 55
TEL;fax:+33 1 44 27 70 00
URL:http://www-spi.lip6.fr/~queinnec/
TITLE:Pr
NOTE:Directeur de la licence d'informatique
X-MOZILLA-HTML:FALSE
END:vcard
\n" )

;;; Should now be encoded as a mime part:
(defsignature paracamplus-vcard
  "\n
BEGIN:vcard
VERSION:2.1
FN:Christian Queinnec
N:Queinnec;Christian
ORG;quoted-printable:Soci=C3t=C3 Paracamplus
ADR:;;7, rue Viollet-le-Duc;Paris;;75009;France
EMAIL;internet:Christian.Queinnec@paracamplus.com
ROLE;quoted-printable:G=C3rant
URL:http://www-spi.lip6.fr/~queinnec/
TITLE;quoted-printable:G=C3rant
NOTE:Soci=C3t=C3 sp=C3cialis=C3e dans l'=C3dition de ressource p=C3dagogiques para-universitaires
X-MOZILLA-HTML:FALSE
END:vcard
\n" )
...
(defun sigadapt-search-record (to)
  ...
  (let* ((data (mail-extract-address-components to))
    ...
    ;; bbdb 2.35 changed the return of bbdb-search-simple to a list or
    ;; maybe I did not notify it before :(
    (let ((records (bbdb-search-simple name net)))
      (if (consp records)
          (car records)
        records ) ) ) )
