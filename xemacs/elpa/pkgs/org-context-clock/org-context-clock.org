        ...
        (let* ((org-log-note-clock-out nil)
          ...
          (let ((prev-clock-buff-read-only
            ...
            (when prev-org-clock-marker
              (org-insert-log-note prev-org-clock-marker (format "clockingout to clockin to %s" new-heading)))

            (with-current-buffer (marker-buffer new-marker)
              (let ((buffer-read-only nil))
                (when old-heading
                  (org-insert-log-note new-marker (format "clockin to here from last clock %s" old-heading)))
