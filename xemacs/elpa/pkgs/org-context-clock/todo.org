...
***** TODO BUG (org-context-clock-add-context-to-org-heading-when-idle) and (org-clock-lotus-log-note-on-change) both tries to capture mini-buffer together
...
***** TODO Not all ORG file should be considered for time tracking todo, find way to limit them at present lot of unnecessary heading shown for all operation
      many file are simple data or log, or just for information purpose there is no meaning of TODO
      device a way to limit them.
***** Believe we should only consider heading without child or node heading for clocking else we should make it configurable. detail
...
***** define a function, so with task heading it will trigger a pre-define action like opening main file in :root directory and it should be able to be used in org agenda buffer also.
