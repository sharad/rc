...
;; Author: BrYan P. Johnson <bilko@onebabyzebra.com>, patched by
...
;; (require 'gnus-pers)
;; (gnus-personality-init) or (gnus-personality-init-for-sending)
...
; + Fixed to use message-setup-hook rather than
; signature-setup-hook. Much nicer now. One bug, though. If doing a
; followup, can get extraneous Cc:.
; + Michael Livshin <mlivshin@bigfoot.com> : Changed electric headers
; to use nnmail-split-fancy.
; + Changed electric headers to use gnus-pers-split instead, and made
; variables customize-able.
; + Added gnus-personality-menu function to add a menubar entry if the
; buttonized from doesn't work.
; + "Paul D. Smith" <pausmith@nortelnetworks.com> : added
; gnus-personality-choose for choosing personalities via prompt. bound
; to C-c C-p in message mode
; + Fixed extraneous Cc: when doing followup bug. If you're getting
; your Cc: fields accidentally deleted, change gnus-pers-cc-fix to
; nil.
; + Stole message-insert-signature and changed a tad to fix the extra
; newline in signature when switching personalities bug. see
...
you want to use every time you send mail from a group."
  ...
  :group 'Personality
  :type '(repeat (cons :tag "" (string :tag "Personality Name")
		       (list :tag "	Personality Data"
			     (choice :tag "		From" (string) (function) (variable))
...
;;;###autoload
(defun gnus-personality-init-for-sending ()
  "Install Personality functionality into message mode. The
personnality will act on sending the message (C-c C-c ...)"
  (add-hook 'message-send-hook
	    'gnus-pers-choose-personality-on-sending)
  )

(if (not (fboundp 'replace-in-string))
    (defun replace-in-string (STR REGEXP NEWTEXT &optional LITERAL)
      (replace-regexp-in-string REGEXP NEWTEXT STR LITERAL)))

(defun gnus-pers-generate-headers-alist ()
  (save-restriction
    ...
    (goto-char (point-min))
    (gnus-pers-generate-headers-alist-aux)))

(defun gnus-pers-generate-headers-alist-aux ()
  (if (eq (point) (point-max))
      '()
    (let* ((beginning-of-name (point))
	   (beginning-of-value
	    (progn
	      (search-forward ": ")
	      (point)))
	   (end-of-name (- beginning-of-value 2))
	   (end-of-value (progn (end-of-line) (point))))
      (forward-char 1)
      (cons
       (cons (make-symbol 
	      (buffer-substring-no-properties beginning-of-name end-of-name))
	     (buffer-substring-no-properties beginning-of-value end-of-value))
       (gnus-pers-generate-headers-alist-aux)))))

(defun gnus-pers-choose-personality-on-sending ()
  (gnus-personality-electric-headers (gnus-pers-generate-headers-alist)))
