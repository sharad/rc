// from http://www.angel.net/~nic/passwdlet.html MD5
...
function str2binl(str){
    ...
    for(var i = 0;
        i < str.length * 8;
        i += 8)bin[i>>5] |= (str.charCodeAt(i / 8) & mask) << (i%32);
    return bin;
}
...
function doIt(I) {
    var master=I.window.prompt('Enter your master password');
    if (master != '' && master != null) {
        re = new RegExp("https*://([^/]+)");
...
interactive("passpass",
            "Password from Master Password.",
            function(I) {
                doIt(I);
            });
