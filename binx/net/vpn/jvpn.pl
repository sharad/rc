...
sub connect_vpn {
  ...
  if ($hostchecker) {
    ...
    $ua->agent('Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:39.0) Gecko/20100101 Firefox/39.0');
...
sub connect_hook {
  ...
  local *FH;
  open(FH, ">$tmpfile") || die "can not open $tmpfile\n";
  ...
  print FH  "# Dynamic resolv.conf(5) file for glibc resolver(3) generated by resolvconf(8)
