...
(defun lotus-messaging/post-init-erc-config ()
  (progn
    (progn
    ...
    (progn
      ...
      (defun erc-bnc4free ()
        ...
        (sleep-for 0 500)
        (lotus-messaging-bnc4free-connect)))
    ...
    (progn
      ...
      ;; (message "loading defun lotus-messaging-start-or-switch")

      (defun lotus-messaging-start-or-switch ()
...
(defun lotus-messaging/init-erc-join-config ()
  (progn
    (progn
      (erc-autojoin-mode t))))

(defun lotus-messaging/init-erc-services-config ()
  (progn
    (require 'passwds)
    (erc-services-mode 1)
...
(defun lotus-messaging/init-erc-nick-notify-config ()
  (progn
      (progn
      ...
      (progn ;; "ERC Notify"
        ...
        (defun notify-desktop (title message &optional duration icon)
        ...
        (progn ;; "monitor"
          ...
          (defvar lotus-messaging-monitor-user-list nil "list")
...
If SERVER is non-nil, use that, rather than the current server."
            ...
            (let ()
              ...
              (push user lotus-messaging-monitor-user-list))
            t)
          ...
          (defun erc-notify-MONITOR (proc parsed)
            (let ((nick (car (erc-parse-user (erc-response.sender parsed))))
                  (target (car (erc-response.command-args parsed)))
                  (msg (erc-response.contents parsed)))
              (when (member nick lotus-messaging-monitor-user-list)
                ;;Handle true private/direct messages (non channel)
                (when (and (not (erc-is-message-ctcp-and-not-action-p msg))
                           (erc-current-nick-p target)
                           (erc-notify-allowed nick target))
                ...
                (when (and (not (erc-is-message-ctcp-and-not-action-p msg))
                           (string-match (erc-current-nick) msg)
                           (member nick lotus-messaging-monitor-user-list)
                           (erc-notify-allowed nick target))
                                        ;Do actual notification
                  ;; (message "ttttttttt")
                  (ding)
...
(defun lotus-messaging/init-h4x0r-config ()
  (progn
    (progn ;; "h4x0r"
...
(defun lotus-messaging/init-bitlbee-config ()
  (progn
...
(defun lotus-messaging/init-circ-config ()
