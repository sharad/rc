<xsl:stylesheet
  ...
  <xsl:variable name="cppvalidchars" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_0123456789'" />
  ...
  <xsl:template match="mn:entry" mode="uid_name_print">
    <xsl:text>/*</xsl:text>
    <xsl:value-of select="$space"/>
    <xsl:text>MELF_FMT_</xsl:text>
    ...
    <xsl:value-of select="$space"/>
    <xsl:text>search for uid=&quot;</xsl:text>
    <xsl:value-of select="@uid"/>
    <xsl:text>&quot; in src/wnc/nms/Melf/xml/melf.xml</xsl:text>
    <xsl:value-of select="$space"/>
    <xsl:text>*/</xsl:text>
  </xsl:template>
...
<xsl:value-of select="$newline"/>
  ...
  <xsl:template match="mn:entry">
    ...
    <xsl:apply-templates select="." mode="uid_name_print"/>
    <xsl:value-of select="$newline"/>
